...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../resnet50_retinaface.wts");
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
    ...
    Int8EntropyCalibrator2 *calibrator = new Int8EntropyCalibrator2(1, INPUT_W, INPUT_H, "/workspace/dataset/benchmarch_data/widerface/val/calib/", "r50_int8calib.table", INPUT_BLOB_NAME);
...
int main(int argc, char** argv) {
    ...
    if (std::string(argv[1]) == "-s") {
        ...
        // std::ifstream file("/workspace/projects/Pytorch_Retinaface/weights_engine/resnet50_1_320_retinaface_int8.engine", std::ios::binary);
        if (file.good()) {
